...
class Model(object):
    def __init__(self):
        ...
        # print("self.X_OP_real", self.X_OP_real.shape)
        self.spanlist = []
        ...
        # self.MF_Processed_real = Weight_Transform(self.MF_real, k=27, n=N, m=M)
        #
        # self.MF_image = tf.Variable(MF_image_init, trainable=True, dtype=tf.float32, name='MF_Image')  # Filter Initialization Needed
        # self.MF_Processed_image = Weight_Transform(self.MF_image, k=27, n=N, m=M)
        #
...
class Span(object):
    ...
    def get_w1(self):
        return self.layerlist[0].get_w1()
...
class Linear_W1(object):
    def __init__(self, X_real, X_image, layer_number, span_number):
        ...
        self.W1_Processed_real = Weight_Transform(self.W1_real, k=28, n=N, m=N)
        ...
        self.W1_Processed_image = Weight_Transform(self.W1_image,k=28, n=N, m=N)
...
class NonLinear_OP(object):
    def __init__(self, S_real, S_image, layer_number, span_number):
        ...
        self.alph = tf.Variable(-1.0*76.32, trainable=True, dtype=tf.float32, name=self.name)              # Alpha Initialization
...
def my_init(shape, dtype=None, partition_info=None):
    val = np.zeros((2*M, M))
    row_index = np.arange(0,2*M,2)
    colum_index = np.arange(M)
